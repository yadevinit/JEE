cDecimalDigits <- 3 # Coz it's unclear where it's supposed to be defined!
# 1: ï»
...
getJADdata <- function(suffix=paste0("till", cYPtill), # was: suffix=paste0("-till", cYPtill)
  ...
  print(str(datmeta2))
  # datmeta2$YPint <- YPintvec(as.character(datmeta2$YP)) # for continuous-time modeling data as input to STM later.
...
includeUIKinflect <- function(JEEdata.use){
  # source(paste0(cRepo.src, "/src-kneeCRLmarksHelp.R"))
  dhxyp <- cbind(JEEdata.use, rOtot=JEEdata.use$right / JEEdata.use$tot)
  dhxyp <- dhxyp[,c("rOtot", "YP")] # ; dhxyp$YP <- as.character(dhxyp$YP)
  print(str(dhxyp))
  thisScurve <- TRUE
  rDifficultyRank <- sort(100*(1:nrow(dhxyp)) / nrow(dhxyp), decreasing=(! thisScurve))
    # decreasing=TRUE results in a convex/concave sigmoidicity that seems to ease estimating the UIKnees
    # and Inflection Point.
  dhxyp <- cbind(dhxyp[order(dhxyp$rOtot, decreasing=thisScurve),],
    rDifficultyRank=rDifficultyRank)
  dhxyp <- dhxyp[, c("rDifficultyRank", "rOtot", "YP")]; str(dhxyp); plot(dhxyp[,1], dhxyp[,2])
  diffi.UIKnee.inflection <- getKneeInflectionPlotYears(dhxyp, xylab=colnames(dhxyp), isScurve=thisScurve)
  print(diffi.UIKnee.inflection)
  rOtotGrp80 <- myCategorize(xvec=(JEEdata.use$right / JEEdata.use$tot), # Beware: dhxyp$rOtot is sorted differently!
    # Coz no JEEdata.use$rOtot direct and JEEdata.use$rOtot3 is imprecise.
    redPreProb=round(1 - diffi.UIKnee.inflection[1]/100, 2))
    # group based on the greater slope-altering UIKnee, rather than the more difficult Inflection. Was: redPreProb=0.25
  JEEdata.use.UIKinflect <- cbind(JEEdata.use, rOtotGrp80=rOtotGrp80)
  # print(summary(JEEdata.use.UIKinflect))
  return(JEEdata.use.UIKinflect)
}
doGroup.qaType <- function(qat){ # Map to {MultCorrAns, SingCorrAnsComprParaTwolist, CreateNewAnsNumInt}.
  grp <- switch(qat,
    MultCorrAns = "MultCorrAns",

    SingCorrAns = "SingCorrAnsComprParaTwolist",
    Compr = "SingCorrAnsComprParaTwolist", # Beware: just 6*3 observations across PCM subjects!
    ParaSingleAns = "SingCorrAnsComprParaTwolist",
    TwoListMatchSingleAns = "SingCorrAnsComprParaTwolist",

    NumAns = "CreateNewAnsNumInt",
    SingDigitInteger = "CreateNewAnsNumInt",

    stop(paste0("unexpected qaType=", qat))
    # Was: "TwoListNumAnsCompr"
    # Was: "notMultSingCorrDigit" # by default ie for c("", "Compr", "NumAns", "TwoListMatchSingleAns")
  )
  return(grp)
}
